...
fun OutsideTeamSelectionScreen(
    ...
    Column(
        ...
        Text(
            text = stringResource(R.string.outside_team_title),
            style = MaterialTheme.typography.headlineMedium,
            modifier = Modifier.padding(top = 24.dp, bottom = 32.dp)
        )
        if (uiState.hasOutsideTeam) {
            Text(
                text = stringResource(R.string.outside_team_yes_description),
                style = MaterialTheme.typography.bodyMedium,
                modifier = Modifier.padding(bottom = 16.dp)
            )
        } else {
            Text(
                text = stringResource(R.string.outside_team_no_description),
                style = MaterialTheme.typography.bodyMedium,
                modifier = Modifier.padding(bottom = 16.dp)
            )
        }
